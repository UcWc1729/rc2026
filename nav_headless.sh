#!/bin/bash
# 导航模式 - 无图形界面启动（SSH/无显示器可用，各节点在后台运行）
# 不修改 nav.sh，此为独立脚本
...
for cmd in "${cmds[@]}"; do
  i=$((i + 1))
  logname="launch_log/nav_$(printf '%02d' $i).log"
  echo "Current CMD : $cmd"
  echo "  -> log: $logname"
  nohup bash -c "cd '$PWD' && source install/setup.bash && $cmd" >> "$logname" 2>&1 &
  sleep 1
done
...
echo ">>> 查看日志: tail -f launch_log/nav_*.log"
